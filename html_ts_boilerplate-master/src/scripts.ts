...
export interface Product {
  ...
  function filterCourses(products: Product[]): Object {
    return products.filter(product => product.type === 'Course')
  }
  
  function getTitles(products: Product[]): Object {
  ...
  const price1 = formatPrice(products[0])
...
//   Everyday Types
  ...
  const anyProduct: RandomProducts = {title: 'Mr. Fluff', kind: 'British Shorthair', price: 4}
  ...
  //  Why?
  const getCurrency = (account: Account) => account.currency
  ...
  const usd: USD | Currency = firstCurrency;
  const eur: EUR | Currency = secondCurrency;
